...
│   │   ├── hooks/                   # ✅ component専用hook（新規）
│   │   │   └── use{ComponentName}Logic.ts
│   │   ├── {ComponentName}.tsx
│   │   └── index.ts
│   └── [id]/                        # 詳細画面用
│       └── hooks/
│           └── use{PageName}Detail.ts
```

**重要な規約:**

- `src/screens/{page-name}/hooks/`: 画面全体で使用されるhook（画面レベルのロジック）
- `src/screens/{page-name}/components/hooks/`: 特定コンポーネント専用のhook（コンポーネントレベルのロジック）
- component専用hookは、そのコンポーネントと強く結びついているため、componentsディレクトリ配下のhooksに配置する
- 命名規則: `use{ComponentName}Logic.ts`（例: `useVideoPlayerLogic.ts`）
